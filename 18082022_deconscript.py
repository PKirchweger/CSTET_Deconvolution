...
def input_files(file_to_work_on, path):
    scriptname = '18082022_deconscript.py'
...
def de_para():
    ...
    with open(inp['logfile'], 'a') as logf:
        psf_file = os.path.join(homedir + 'PSFs' + '/PSF_' + test_dict['pix_nm'] + 'nm.mrc')
            ...
            for lines in inpf:
                ...
                if 'Do_summed_PS' in lines:
                    if 'Yes' in lines:
                        ...
                        exit()
                if 'Threads' in lines:
                    threads = str(lines.split()[2])
                    decon_para['np'] = threads
...
def fan_gen():
    with open(inp['logfile'], 'a') as logf:
        # CHOOSE THE RIGHT PSF
        with open(decon_para['tlt_file'], 'r') as degfile:
            for line in degfile:
                deg = line.strip()
                rot_cmd = 'rotatevol -memory ' + decon_para['mem'] + ' -FillValue 0 -OutputSizeXYZ ' + test_dict['xy'] + ','+ test_dict['xy'] + ',' + test_dict['z'] + ' -a 0,' + deg + ',0 -i ' + decon_para['PSF'] + ' -ou rotvol/PSF_' + deg + 'degY.mrc'
                subprocess.run(rot_cmd, shell=True)
                logf.write(rot_cmd + '\n')
...
def fan_gen_b():
    with open(inp['logfile'], 'a') as logf:
        ...
        with open(decon_para['b_tlt_file'], 'r') as degfile:
            for line in degfile:
                deg = line.strip()
                rot_cmd = 'rotatevol -memory ' + decon_para['mem'] + ' -FillValue 0 -OutputSizeXYZ ' + test_dict['xy'] + ','+ test_dict['xy'] + ',' + test_dict['z'] + ' -a 0,' + deg + ',0 -i ' + decon_para['PSF'] + ' -ou rotvol_b/PSF_' + deg + 'degY.mrc'
                subprocess.run(rot_cmd, shell=True)
                logf.write(rot_cmd + '\n')
        ...
        for it in decon_para['ni']:
            for smooth in decon_para['smooth']:
                for lamsmooth in decon_para['lamsmooth']:
                    ...
                    decon_cmd = 'core2_decon ' + inp['scaletomo'] + ' ' + decon_out + ' ' + inp['OTF_file'] + ' -alpha=100 -lamratio=0:1 -lamf=' + str(smooth) + ' -lampc=0 -lampos=1 -lamsmooth=' + str(lamsmooth) + ' -laml2=0 -cuth=0.001 -ncycl=' + str(it) + ' -omega=0.8 -sub=1:1:1:1:1 -tol=0.0001 -np=' + decon_para['np'] + ' -oplotfile=' + plotfile + ' -linesearch="2014" -regtype="ma" -logging=progress'

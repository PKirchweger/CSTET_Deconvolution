...
def environment():
    ...
    if not str('clip') in str(clip_pr):
        ...
        print('load IMOD with "module load cuda/10.2 IMOD/4.12.10"')
...
def write_log_file(logfile):
  if logfile in os.listdir('.'): 
    with open(logfile, 'a') as logf:
      ...
      logf.write("##### The time is " + asctime() + "#####\n")
...
def input_files(file_to_work_on, path):
    scriptname = '20250106_deconscript_parallel.py'
...
def de_para():
    ...
    with open(inp['logfile'], 'a') as logf:
        psf_file = os.path.join('/storwis/Labs/cpelbaum/peterkir/PSFs' + '/PSF_' + test_dict['pix_nm'] + 'nm.mrc')
            ...
            for lines in inpf:
                ...
                if 'Needs_Invertion' in lines: 
                    if 'Yes' in lines: 
                        ...
                        exit()
                if 'Do_Scaling' in lines:
                    if 'Yes' in lines:
                        decon_para['do_scale'] = 'Yes'
                    elif 'No' in lines:
                        decon_para['do_scale'] = 'No'
                    else:
                        print('Please write "Do_Scaling = Yes" or "Do_Scaling = No" in the input_values.txt')
                        exit()
...
def extract_data(inputfile):
    try:
        with open(inp['logfile'], 'r+') as logf:
            tomoheader = subprocess.run(['header', inputfile], capture_output=True, text=True)  # read the header into stdout
            logf.write(tomoheader.stdout)  # write the header into the logfile
            extracted_values = {}  # create a dictonary
            # Split the output into lines and search for the required values
            lines = tomoheader.stdout.split('\n')
            for line in lines:
                try: 
                    if 'Minimum density' in line:
                        extracted_values['mini'] = line.split()[-1]
                    if 'Maximum density' in line:
                        extracted_values['maxi'] = line.split()[-1]
                    if 'Pixel spacing' in line:
                        extracted_values['pix_A'] = str(round(float((line.split()[-1])), 2))
                        extracted_values['pix_nm'] = str(round((float((line.split()[-1])) / 10), 3))
                    if 'Number of columns' in line:
                        extracted_values['x'] = line.split()[-3]
                        extracted_values['y'] = line.split()[-2]                        
                        extracted_values['z'] = line.split()[-1]
                except (IndexError, ValueError) as e:
                    print(f"Error processing line '{line}': {e}")
            # write a summary into the logfile
            logf.write('\n### These are the values I read from the header of the tomogram: ###\n')
            logf.write(f"Minmum = {extracted_values['mini']}\n")
            logf.write(f"Maximum = {extracted_values['maxi']}\n")
            logf.write(f"Pixel size (A) = {extracted_values['pix_A']}\n")
            logf.write(f"Pixel size (nm) = {extracted_values['pix_nm']}\n")
            logf.write(f"Image size is = {extracted_values['x']},{extracted_values['y']},{extracted_values['z']}\n")
...
def fan_gen(n_cores):
    with open(inp['logfile'], 'a') as logf:
        ...
        for i in range(0,len(deg)):
            rot_cmd = 'rotatevol -memory ' + decon_para['mem'] + ' -FillValue 0 -OutputSizeXYZ ' + test_dict['x'] + ','+ test_dict['y'] + ',' + test_dict['z'] + ' -a 0,' + str(deg[i]) + ',0 -i ' + decon_para['PSF'] + ' -ou rotvol/PSF_' + str(deg[i]) + 'degY.mrc'
            logf.write(rot_cmd + '\n')
...
def fan_gen_b(n_cores):
    with open(inp['logfile'], 'a') as logf:
        ...
        params = []
        for i in range(0,len(deg)):
            rot_cmd = 'rotatevol -memory ' + decon_para['mem'] + ' -FillValue 0 -OutputSizeXYZ ' + test_dict['x'] + ','+ test_dict['y'] + ',' + test_dict['z'] + ' -a 0,' + str(deg[i]) + ',0 -i ' + decon_para['PSF'] + ' -ou rotvol_b/PSF_' + str(deg[i]) + 'degY.mrc'
            logf.write(rot_cmd + '\n')
            params.append(rot_cmd)
        processes = ()
        for i in range(0,len(params)):
...
def ftransform():
    with open(inp['logfile'], 'a') as logf:
        if not os.path.isfile(inp['OTF_file']):
            ...
            new_x = str(int(test_dict['x']) - 1)
            new_y = str(int(test_dict['y']) - 1)
            new_z = str(int(test_dict['z']) - 1)
            if int(test_dict['x']) % 2 == 0:
                shift_x = int(test_dict['x']) / 2
            else:
                shift_x = (int(test_dict['x']) - 1) / 2
            shift_x = str(round(shift_x, 0))
            if int(test_dict['y']) % 2 == 0:
                shift_y = int(test_dict['y']) / 2
            else:
                shift_y = (int(test_dict['y']) - 1) / 2
            shift_y = str(round(shift_y, 0))
            ...
            ft_cmd = 'FTransform3D ' + inp['Fan_file'] + ' ' + inp['OTF_file'] + ' -x1=0:' + new_x + ' -y1=0:' + new_y + ' -z1=0:' + new_z + ':1 -w1=0 -t1=0:0:1 -res1=0 -mode1=complex -title='' -real_complex -same_units -shift=' + shift + ' -xpad=default -xpad_value=0:0 -ypad=default -ypad_value=0:0 -zpad=default -zpad_value=0:0 -3d=z'
            logf.write(ft_cmd + '\n\n')
    ...
    with open(inp['logfile'], 'a') as logf:
        if not os.path.isfile(inp['scaletomo']):
            ...
            if decon_para['do_scale'] == 'Yes':
                ...
                logf.write("##### I have scaled the density and changed the mode to 32bit floating point with this command: #####'\n'")
                logf.write(scale_cmd + '\n\n')
            elif decon_para['do_scale'] == 'No':
                inp['scaletomo'] = inp['invtomo']
...
def decon():
    with open(inp['logfile'], 'a') as logf:
        logf.write('### I will run deconvolution runs with' + '\n')
...
def histo(in_file):
    with open(inp['logfile'], 'a') as logf:
        ...
        percentile_cmd = 'mrcpercentiles ' + in_file + ' -cut=0.0001 -cut=0.9999'
        ...
        perc_out = subprocess.run(['grep', test_dict['y']], capture_output=True, text=True, input=percentile.stdout)
